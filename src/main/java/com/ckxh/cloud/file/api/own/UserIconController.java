...
@Scope("singleton")
@Controller
@RequestMapping("/own/icon")
@AuthPathOnBind("get:/platformApi/own/user/normal/#")
public class UserIconController extends FileLoader {
	...
	public String upload(@RequestPart MultipartFile file, HttpServletRequest request, HttpServletResponse response){
		if (file != null && !file.isEmpty() && file.getSize() > 0) { 
			if(file.getSize() / 1024 > 30){
				return JsonUtil.createSuccessJson(false, null, "invalid", "max file size 30KB");
			}
            ...
            try {
            	...
            	String filePfx = "upload/icon/" + DateUtil.convertDateToString(new Date(), DateUtil.Date_YMD_NoSep) + "/" + uid + "/";
